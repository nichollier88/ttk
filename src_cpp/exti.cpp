...
void i_draw(TWidget* this, ttk_surface srf) {
    ttk_text(srf, ttk_textfont, this->x + 3,
             ...
             ttk_makecol(BLACK), "Type to input, ENTER when done.");
}
...
void t_draw(TWidget* this, ttk_surface srf) {
    if (inputting && !strlen(thestring))
        ttk_text(srf, ttk_menufont, this->x, this->y, ttk_makecol(BLACK),
                 "Nothing yet.");
    else if (inputting)
        ttk_text(srf, ttk_textfont, this->x, this->y, ttk_makecol(BLACK),
                 thestring);
    else
        ttk_text(srf, ttk_menufont, this->x, this->y, ttk_makecol(BLACK),
                 "Action to input.");
