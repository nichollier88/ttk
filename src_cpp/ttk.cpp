...
int ttk_version_check(int otherver) {
    ...
    if (myver < otherver) {
        fprintf(stderr,
                "Error: I was compiled with TTK headers version %x but "
    ...
    if (((myver & ~0xff) == (otherver & ~0xff)) &&
        ...
        fprintf(stderr,
                "Warning: I was compiled with TTK headers version %x but "
                "linked with library version %x.\n         "
...
TWindow* ttk_init() {
    ...
    if (!ttk_screen) {
        ...
        if (ttk_screen->bpp == 16) {
            if (ttk_screen->h == 176 || ttk_screen->h == 132)
                ttk_screen->wy =
                    21; /* Photo & Nano: 22px header, 22px menu items */
    ...
    if (ttk_screen->bpp == 2) {
        ...
        ttk_draw_icon_color(ttk_icon_tux, ttk_screen->srf,
                            ...
                            dots_1, dots_b);
        ttk_draw_icon_color(ttk_icon_tux, ttk_screen->srf,
                            (ttk_screen->w * 3 / 4) - 16 + 2,
                            ttk_screen->h / 2 - 16, dots_1, dots_b);
    } else {
        ttk_fillellipse(ttk_screen->srf, ttk_screen->w / 4, ttk_screen->h / 2,
                        18, 18, dots_1);

        ttk_fillellipse(ttk_screen->srf, ttk_screen->w / 2, ttk_screen->h / 2,
                        18, 18, dots_2);
        ttk_fillellipse(ttk_screen->srf, ttk_screen->w * 3 / 4,
                        ttk_screen->h / 2, 18, 18, dots_3);
        ...
        ttk_draw_icon_color(ttk_icon_tux, ttk_screen->srf,
                            (ttk_screen->w * 3 / 4) - 16 + 2,
                            ttk_screen->h / 2 - 16, dots_b, dots_3);
    }
    ...
    if (!nfonts) {
        fprintf(
            stderr,
            "No fonts list, or no fonts in it. Make one, or put some in.\n");
        ttk_quit();
        exit(1);
    }
...
static int iterate_widgets(TWidgetList* wids, int (*func)(TWidget*, int),
                           int arg) {
...
static int check_dirty(TWidget* wid, int unused) { return !!wid->dirty; }
    ...
    while (1) {
        ...
        if (ev == TTK_BUTTON_DOWN) {
            if (!((ttk_button_pressedfor[earg] == 0 ||
                  ...
                  (ttk_button_presstime[earg] == 0 ||
                   ttk_button_presstime[earg] == tick)))  // key rept
                global = 0;
        }
        ...
        switch (ev) {
            case TTK_BUTTON_DOWN:
                if (!ttk_button_presstime[earg] ||
                    !ttk_button_pressedfor[earg]) {  // don't reset with
                                                     // key-repted buttons
                ...
                // Don't send different parts of same keyt-rept to different
                // widgets:
            ...
            case TTK_BUTTON_UP:
                ...
                // Need to be before, in case button() launches its own
                // ttk_run().
                ttk_button_presstime[earg] = 0;
                ttk_button_holdsent[earg] = 0;
        ...
        if (evtarget) {
            ...
            for (p = keys; *p; p++) {
                ...
                // If user pushes a button on clickwheel, it'll also register as
                // a tap. Counter that.
                if (ttk_button_presstime[*p] &&
            ...
            if (ttk_get_podversion() & TTK_POD_PP502X) {
                ...
                if (st) {
                    ...
                    // Heuristic: finger moved <1/20 of wheel, held for <2/5
                    // sec.
        ...
        if (!ttk_windows) return 0;
        win = ttk_windows->w;  // Yes, again; ttk_show_window() might've been
                               // called by an event.
        ...
        if ((ttk_dirty & TTK_DIRTY_HEADER) && win->show_header) {
            const char* displayTitle =
                ttk_filter_sorting_characters(win->title);
            /* Clear it */
            ...
            /* Draw the widgets */
            if (ttk_header_widgets)
                iterate_widgets(ttk_header_widgets, do_draw, 1);

            /* Draw title */
            /* autocenter if unset */
            textpos =
                ((header_text_pos >= 0) ? header_text_pos : ((s->w) >> 1));
            ...
            switch (header_text_justification) {
                ...
                default:
                    textpos -=
                        (ttk_text_width(ttk_menufont, displayTitle) >> 1);
                    break;
            }
...
void ttk_click_ex(int period, int duration) {
    period = period ? MIN(MAX(period, 5), 250)
                    : 20;  // period: 40=2286Hz, 30=3024Hz, 20=4465Hz, 10=8547Hz
    ...
    if (ttk_get_podversion() & TTK_POD_PP502X) {
        ...
        outl(0x80000000 | 0x800000 | (period & 0xffff),
             0x7000a000); /* set pitch */

        int starttime = C_timer_get_current();
        while (!C_timer_check(
            starttime,
            duration * 1000));  // milliseconds to microseconds (ms*1000)
...
void ttk_update_lcd(int xstart, int ystart, int xend, int yend,
    ...
    if (lcd_base < 0) {
        ...
        if (ver & TTK_POD_SANSA) return;  // SDL handles LCD code
