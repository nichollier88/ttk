...
typedef struct _ttk_font {
    ...
    struct _TTF_Font* tf;
#endif
    void (*draw)(struct _ttk_font*, ttk_surface, int, int, ttk_color,
                 const char*);
    void (*draw_lat1)(struct _ttk_font*, ttk_surface, int, int, ttk_color,
                      const char*);
    void (*draw_uc16)(struct _ttk_font*, ttk_surface, int, int, ttk_color,
                      const uc16*);
...
typedef struct ttk_fontinfo {
    char file[64]; /* font file, relative to /usr/share/fonts on iPod, fonts/ on
                      X11 */
...
/* Implemented by TTK core: */ ttk_color ttk_mapcol(ttk_color col,
                                                    ttk_surface src,
                                                    ttk_surface dst);
...
#define TTK_SCROLL_ACCEL(ttk_accel_event, ttk_accel_slow, ttk_accel_max)      \
    do {                                                                      \
        ...
        if (ttk_accel_now > TTK_ACCEL_H ||                                    \
            ttk_accel_throttle * ttk_accel_event < 0) {                       \
            ttk_accel_throttle =                                              \
                0; /* user changed direction or timed out; stop accel */      \
        } else if (ttk_accel_now > TTK_ACCEL_L &&                             \
                   abs(ttk_accel_throttle) >                                  \
                       abs(ttk_accel_event)) { /* slow the accel */           \
            ttk_accel_throttle -= ttk_accel_event;                            \
        } else { /* increase accel coefficient up to max */                   \
            ttk_accel_throttle +=                                             \
                ttk_accel_event; /* count can be negative, meaning backwards  \
                                    direction*/                               \
        }                                                                     \
        ttk_accel_event *=                                                    \
            MIN(abs(ttk_accel_throttle / ttk_accel_slow) + 1, ttk_accel_max); \
