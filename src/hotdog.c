...
static void update2(hd_engine* e, int x, int y, int w,
                    int h) /* translates from 2bpp */
{
    static Uint16 colors[] = {/* White   */ 0xFFFF,
                              /* Lt.Grey */ 0xA514,
                              /* Dk.Grey */ 0x528A,
                              /* Black   */ 0x0000};
    ...
    for (line = y; line < y + h; line++) {
        int pi = x / 4, pp = x % 4;  // pi is the byte, pp is the pixel within
                                     // the byte, in the FB
        int qi;
        for (qi = x; qi < x + w; qi++) {
...
void ttk_gfx_init() {
    ...
    SDLscreen =
        SDL_SetVideoMode(ttk_screen->w, ttk_screen->h, 16, SDL_SWSURFACE);
    if (!SDLscreen) {
    ...
    ttk_engine =
        HD_Initialize(ttk_screen->w, ttk_screen->h, ttk_screen->bpp,
                      framebuffer, (ttk_screen->bpp == 2) ? update2 : update16);
#else /* iPod: */
    HD_LCD_Init();
    if (ttk_screen->bpp == 2)
        framebuffer = malloc(ttk_screen->w * (ttk_screen->h + 3 / 4));
    ...
    ttk_engine = HD_Initialize(ttk_screen->w, ttk_screen->h, ttk_screen->bpp,
                               framebuffer, update);
...
void ttk_gfx_update(ttk_surface srf) { HD_Render(ttk_engine); }
...
int ttk_get_event(int* arg) {
    // IKB can report multiple button events at once, we only want one at a
    // time, so save the rest.
    static int pending_presses, pending_releases;
    if (pending_presses || pending_releases) {
    handle_button:;  // Control is transferred here after receiving a button
                     // event, too.
        char* buttons = "\nfwdmh";  // button 0 is (1 << 0) = 1 in the mask and
                                    // buttons[0] = '\n' in TTK, etc.
        int i;
        for (i = 0; i < 6; i++) {
    ...
    while (SDL_PollEvent(&ev)) {
        switch (ev.type) {
            ...
            case SDL_KEYUP:
                ...
                tev =
                    (ev.type == SDL_KEYDOWN) ? TTK_BUTTON_DOWN : TTK_BUTTON_UP;
                ...
                else if (ev.key.keysym.sym == SDLK_KP0)
                    *arg = '0';
                else if (ev.key.keysym.sym == SDLK_KP1)
                    *arg = '1';
                else if (ev.key.keysym.sym == SDLK_KP2)
                    *arg = '2';
                else if (ev.key.keysym.sym == SDLK_KP3)
                    *arg = '3';
                else if (ev.key.keysym.sym == SDLK_KP4)
                    *arg = '4';
                else if (ev.key.keysym.sym == SDLK_KP5)
                    *arg = '5';
                else if (ev.key.keysym.sym == SDLK_KP6)
                    *arg = '6';
                else if (ev.key.keysym.sym == SDLK_KP7)
                    *arg = '7';
                else if (ev.key.keysym.sym == SDLK_KP8)
                    *arg = '8';
                else if (ev.key.keysym.sym == SDLK_KP9)
                    *arg = '9';
                return tev;
        }
    }
...
static void poly_func(void (*fn)(hd_surface, hd_point*, int, uint32),
                      ttk_surface srf, int nv, short* vx, short* vy,
                      ttk_color col) {
...
void ttk_aapolyline(ttk_surface srf, int nv, short* vx, short* vy,
                    ttk_color col) {
...
void ttk_ellipse(ttk_surface srf, int x, int y, int rx, int ry, ttk_color col) {
    if (rx == ry)
        HD_Circle(srf, x, y, rx, col);
    else
        HD_Ellipse(srf, x, y, rx, ry, col);
}
void ttk_ellipse_gc(ttk_surface srf, ttk_gc gc, int x, int y, int rx, int ry) {
    if (rx == ry)
        HD_Circle(srf, x, y, rx, gc->fg);
    else
        HD_Ellipse(srf, x, y, rx, ry, gc->fg);
}
void ttk_aaellipse(ttk_surface srf, int x, int y, int rx, int ry,
                   ttk_color col) {
    if (rx == ry)
        HD_AACircle(srf, x, y, rx, col);
    else
        HD_AAEllipse(srf, x, y, rx, ry, col);
}
void ttk_aaellipse_gc(ttk_surface srf, ttk_gc gc, int x, int y, int rx,
                      int ry) {
    if (rx == ry)
        HD_AACircle(srf, x, y, rx, gc->fg);
    else
        HD_AAEllipse(srf, x, y, rx, ry, gc->fg);
}
void ttk_fillellipse(ttk_surface srf, int x, int y, int rx, int ry,
                     ttk_color col) {
    if (rx == ry)
        HD_FillCircle(srf, x, y, rx, col);
    else
        HD_FillEllipse(srf, x, y, rx, ry, col);
}
void ttk_fillellipse_gc(ttk_surface srf, ttk_gc gc, int x, int y, int rx,
                        int ry) {
    if (rx == ry)
        HD_FillCircle(srf, x, y, rx, gc->fg);
    else
        HD_FillEllipse(srf, x, y, rx, ry, gc->fg);
}
void ttk_aafillellipse(ttk_surface srf, int x, int y, int rx, int ry,
                       ttk_color col) {
    if (rx == ry)
        HD_AAFillCircle(srf, x, y, rx, col);
    else
        HD_AAFillEllipse(srf, x, y, rx, ry, col);
}
void ttk_aafillellipse_gc(ttk_surface srf, ttk_gc gc, int x, int y, int rx,
                          int ry) {
    if (rx == ry)
        HD_AAFillCircle(srf, x, y, rx, gc->fg);
    else
        HD_AAFillEllipse(srf, x, y, rx, ry, gc->fg);
}
...
void ttk_text_gc(ttk_surface srf, ttk_gc gc, int x, int y, const char* str) {
    if (gc->usebg)
        ttk_fillrect(srf, x, y, x + ttk_text_width(gc->font, str),
                     y + gc->font->f->h, gc->bg);
...
int ttk_text_height(ttk_font fnt) { return fnt->f->h; }
int ttk_text_height_gc(ttk_gc gc) { return gc->font->f->h; }
...
void ttk_free_image(ttk_surface img) {
    if (img) HD_FreeSurface(img);
}

void ttk_blit_image(ttk_surface src, ttk_surface dst, int dx, int dy) {
    HD_ScaleBlendClip(src, 0, 0, HD_SRF_WIDTH(src), HD_SRF_HEIGHT(src), dst, dx,
                      dy, HD_SRF_WIDTH(src), HD_SRF_HEIGHT(src), 0, 255);
}

void ttk_blit_image_ex(ttk_surface src, int sx, int sy, int sw, int sh,
                       ttk_surface dst, int dx, int dy) {
    HD_ScaleBlendClip(src, sx, sy, sw, sh, dst, dx, dy, sw, sh, 0, 255);
}
...
ttk_surface ttk_scale_surface(ttk_surface srf, float factor) {
    ...
    HD_ScaleBlendClip(srf, 0, 0, sw, sh, ret, 0, 0, dw, dh, 0, 255);
    return ret;
}
...
void ttk_free_surface(ttk_surface srf) { HD_FreeSurface(srf); }
