...
void ttk_gfx_init() {
    ...
    ttk_screen->srf = GrNewWindowEx(
        GR_WM_PROPS_APPFRAME | GR_WM_PROPS_CAPTION | GR_WM_PROPS_CLOSEBOX,
        "TTK Program", GR_ROOT_WINDOW_ID, 0, 0, ttk_screen->w, ttk_screen->h,
        ttk_makecol(WHITE));

    GrSelectEvents(ttk_screen->srf,
...
void ttk_gfx_quit() { GrClose(); }

void ttk_gfx_update(ttk_surface s) { GrFlush(); }
    ...
    if (GrCheckNextEvent(&ev), ev.type != GR_EVENT_TYPE_NONE) {
        switch (ev.type) {
            ...
            case GR_EVENT_TYPE_KEY_UP:
                tev = (ev.type == GR_EVENT_TYPE_KEY_UP) ? TTK_BUTTON_UP
                                                        : TTK_BUTTON_DOWN;
    ...
    while (GrCheckNextEvent(&ev), ev.type != GR_EVENT_TYPE_NONE) {
        switch (ev.type) {
            ...
            case GR_EVENT_TYPE_KEY_UP:
                ...
                tev = (ev.type == GR_EVENT_TYPE_KEY_DOWN) ? TTK_BUTTON_DOWN
                                                          : TTK_BUTTON_UP;

                if (ev.keystroke.ch == 'w' || ev.keystroke.ch == MWKEY_LEFT)
                    *arg = TTK_BUTTON_PREVIOUS;
                else if (ev.keystroke.ch == 'f' ||
                         ev.keystroke.ch == MWKEY_RIGHT)
                    *arg = TTK_BUTTON_NEXT;
                else if (ev.keystroke.ch == 'm' || ev.keystroke.ch == MWKEY_UP)
                    *arg = TTK_BUTTON_MENU;
                else if (ev.keystroke.ch == 'd' ||
                         ev.keystroke.ch == MWKEY_DOWN)
                    *arg = TTK_BUTTON_PLAY;
                else if (ev.keystroke.ch == 'h')
                    *arg = TTK_BUTTON_HOLD;
                else if (ev.keystroke.ch == MWKEY_ENTER)
                    *arg = TTK_BUTTON_ACTION;

#ifndef IPOD
                else if (ev.keystroke.ch == MWKEY_KP0)
                    *arg = '0';
                else if (ev.keystroke.ch == MWKEY_KP1)
                    *arg = '1';
                else if (ev.keystroke.ch == MWKEY_KP2)
                    *arg = '2';
                else if (ev.keystroke.ch == MWKEY_KP3)
                    *arg = '3';
                else if (ev.keystroke.ch == MWKEY_KP4)
                    *arg = '4';
                else if (ev.keystroke.ch == MWKEY_KP5)
                    *arg = '5';
                else if (ev.keystroke.ch == MWKEY_KP6)
                    *arg = '6';
                else if (ev.keystroke.ch == MWKEY_KP7)
                    *arg = '7';
                else if (ev.keystroke.ch == MWKEY_KP8)
                    *arg = '8';
                else if (ev.keystroke.ch == MWKEY_KP9)
                    *arg = '9';
#endif
                else
...
int ttk_getticks() {
    struct timeval tv;
    gettimeofday(&tv, 0);
    return (tv.tv_usec - tvstart.tv_usec) / 1000 +
           (tv.tv_sec - tvstart.tv_sec) * 1000;
}
...
ttk_gc ttk_new_gc() { return GrNewGC(); }
ttk_gc ttk_copy_gc(ttk_gc other) { return GrCopyGC(other); }
...
void ttk_gc_set_font(ttk_gc gc, ttk_font font) { GrSetGCFont(gc, font.f); }
void ttk_gc_set_usebg(ttk_gc gc, int flag) { GrSetGCUseBackground(gc, flag); }
...
void ttk_free_gc(ttk_gc gc) { GrDestroyGC(gc); }
...
int ttk_text_height(ttk_font fnt) { return fnt.inf.height; }
...
void ttk_free_image(ttk_surface img) { GrDestroyWindow(img); }
...
void ttk_blit_image_ex(ttk_surface src, int sx, int sy, int sw, int sh,
    ...
    if (sx + sw > rsw) sw = rsw - sx;
    if (sy + sh > rsh) sh = rsh - sy;
    if (dx + sw > rdw) sw = rdw - dx;
    if (dy + sh > rdh) sh = rdh - dy;
    if (sw > 0 && sh > 0) {
        GrSetGCForeground(tmp_gc, GR_RGB(0, 0, 0));
...
ttk_surface ttk_scale_surface(ttk_surface srf, float factor) {
    ...
    GrStretchArea(ret, tmp_gc, 0, 0, w * factor, h * factor, srf, 0, 0, w, h,
                  MWROP_SRCCOPY);
    return ret;
}
...
void ttk_free_surface(ttk_surface srf) { GrDestroyWindow(srf); }
