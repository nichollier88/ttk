...
#define NOPAR SDL_INIT_NOPARACHUTE
    ...
    if ((ttk_screen->srf = SDL_SetVideoMode(ttk_screen->w, ttk_screen->h, bpp,
                                            vidflags)) == 0) {
...
void ttk_gfx_quit() { SDL_Quit(); }

void ttk_gfx_update(ttk_surface srf) { SDL_Flip(srf); }

/* Polls for a raw SDL event, handling basic key mapping */
...
/* Internal helper to save a surface as a PPM image */
...
/* Saves a screenshot to a file named Screen_XXXX.ppm */
...
/*
 * High-level event retrieval.
 * Maps SDL input (keyboard/mouse) to TTK specific events (buttons, scroll).
 * Includes logic to amalgamate scroll events to reduce lag.
 */
    ...
    while ((ms = SDL_GetTicks()) < time && wait_event(&ev, time - ms)) {
        switch (ev.type) {
            ...
            case SDL_MOUSEBUTTONUP:
                switch (ev.button.button) {
                    case SDL_BUTTON_WHEELUP:
                        tev = TTK_SCROLL;
                        (*arg)--;
                        if (ev.type == SDL_MOUSEBUTTONUP) return tev;
                        break;
            ...
            case SDL_MOUSEBUTTONDOWN:
                ...
                return (ev.type == SDL_MOUSEBUTTONDOWN) ? TTK_BUTTON_DOWN
                                                        : TTK_BUTTON_UP;
#endif
            ...
            case SDL_KEYUP:
                ...
                return (ev.type == SDL_KEYDOWN) ? TTK_BUTTON_DOWN
                                                : TTK_BUTTON_UP;
        }
    }
    return tev;
...
void ttk_fillrect_gc(ttk_surface srf, ttk_gc gc, int x, int y, int w, int h) {
    if (gc->xormode) {
        ...
        if (ttk_screen->bpp == 2) {
            Uint8* startp = (Uint8*)srf->pixels + y1 * srf->pitch +
                            x1;  // start of this line
            Uint8* endp =
                (Uint8*)srf->pixels + y1 * srf->pitch + x2;  // end of this line
            Uint8* Endp =
                (Uint8*)srf->pixels + y2 * srf->pitch + x2;  // end of it all
            Uint8* p;
            ...
            Uint16* startp = (Uint16*)((Uint8*)srf->pixels + y1 * srf->pitch) +
                             x1;  // start of this line
            Uint16* endp = (Uint16*)((Uint8*)srf->pixels + y1 * srf->pitch) +
                           x2;  // end of this line
            Uint16* Endp = (Uint16*)((Uint8*)srf->pixels + y2 * srf->pitch) +
                           x2;  // end of it all
            Uint16* p;

            while (endp <= Endp) {
                p = startp;
                while (p < endp) *p++ ^= 0xffff;
...
/* Draw an anti-aliased ellipse or circle */
...
void ttk_aafillellipse(ttk_surface srf, int xc, int yc, int rx, int ry,
    ...
    /* Lock surface */
    if (SDL_MUSTLOCK(srf)) {
        if (SDL_LockSurface(srf) < 0) return;
    }
...
#define dwswap(x)                                               \
    ((((x) << 24) & 0xff000000L) | (((x) << 8) & 0x00ff0000L) | \
     (((x) >> 8) & 0x0000ff00L) | (((x) >> 24) & 0x000000ffL))
#endif
 ...
 */
static void bit_order_invert(unsigned char* buf, int nbytes) {
    for (; --nbytes >= 0; buf++) *buf = _reverse_byte[*buf];
}

/*
 *	Invert byte order within each 16-bits of an array.
 */
static void two_byte_swap(unsigned char* buf, int nbytes) {
    unsigned char c;
 ...
 */
static void four_byte_swap(unsigned char* buf, int nbytes) {
    unsigned char c;
...
/* read an 8 bit byte*/
static unsigned short readINT8(FILEP file) {
    unsigned char b;
...
static int pcf_readbitmaps(FILE* file, unsigned char** bits, int* bits_size,
    ...
    for (i = 0; i < num_glyphs; ++i) o[i] = readLSB32(file);

    for (i = 0; i < GLYPHPADOPTIONS; ++i) bmsize[i] = readLSB32(file);
...
#if SDL_BYTE_ORDER == SDL_BIG_ENDIAN
    if (endian == PCF_LSB_FIRST) two_byte_swap(b, *bits_size);
#else
    if (endian == PCF_MSB_FIRST) two_byte_swap(b, *bits_size);
#endif
    return num_glyphs;
}
...
static int pcf_readmetrics(FILE* file, struct metric_entry** metrics) {
    ...
    if ((format & PCF_FORMAT_MASK) == PCF_DEFAULT_FORMAT) {
        ...
        m = *metrics =
            (struct metric_entry*)malloc(size * sizeof(struct metric_entry));
...
static int pcf_read_encoding(FILE* file, struct encoding_entry** encoding) {
    ...
    e = *encoding =
        (struct encoding_entry*)malloc(sizeof(struct encoding_entry));
    ...
    e->count =
        (e->max_byte2 - e->min_byte2 + 1) * (e->max_byte1 - e->min_byte1 + 1);
...
static int pcf_read_toc(FILE* file, struct toc_entry** toc,
    ...
    if (version != PCF_FILE_VERSION) return -1;
...
/* Loads a PCF font file. Logic adapted from Microwindows/Free86 PCF code. */
static void load_pcf(Bitmap_Font* bf, const char* fname) {
    ...
    bf->bits = (unsigned short*)calloc(
        (max_height * (sizeof(unsigned short) * bwidth)), glyph_count);
    goffset = (unsigned long*)malloc(glyph_count * sizeof(unsigned long));
    ...
    for (i = 0; i < glyph_count; i++) {
        ...
        int xwidth =
            (metrics[i].rightBearing - metrics[i].leftBearing + 15) / 16;
    ...
    bf->offset =
        (unsigned long*)malloc(encoding->count * sizeof(unsigned long));
    bf->width = (unsigned char*)malloc(encoding->count * sizeof(unsigned char));
...
#endif
else textsrf = TTF_Render##func##_Blended(f->tf, str.col);
...
/*
 * Loads a font from various supported formats.
...
ttk_surface ttk_new_surface(int w, int h, int bpp) {
    ...
    ret = SDL_CreateRGBSurface(SDL_SWSURFACE, w, h, BPP, Rmask, Gmask, Bmask,
                               Amask);
    if (!ret) {
