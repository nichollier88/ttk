...
Application::Application() {
    if (!_initialized) ttk_init();
    _initialized++;
}

Application::~Application() {
    _initialized--;
    if (!_initialized) ttk_quit();
}

void Application::setResolution(int w, int h, int bpp) {
    if (_initialized) {
        fprintf(stderr,
                "Warning: setting resolution to %dx%dx%d after initialization, "
                "probably won't work\n",
                w, h, bpp);
    }
...
int Application::run() { return ttk_run(); }
...
Font::~Font() {
    if (_needsfree) ttk_done_font(_fnt);
}

ttk_fontinfo* Font::info() { return _fnt->fi; }

int Font::width(const char* str) { return ttk_text_width(_fnt, str); }
int Font::width_lat1(const char* str) { return ttk_text_width_lat1(_fnt, str); }
int Font::width_uc16(const uc16* str) { return ttk_text_width_uc16(_fnt, str); }
int Font::height() { return ttk_text_height(_fnt); }

Color::Color(int r, int g, int b) : _col(ttk_makecol(r, g, b)) {}
Color::Color(const char* ap_prop) : _col(ttk_ap_getx(ap_prop)->color) {}
Color::Color(ttk_color col) : _col(col) {}
...
void Color::unmake(int& r, int& g, int& b) { ttk_unmakecol(_col, &r, &g, &b); }

Surface::Surface(int w, int h) : _w(w), _h(h) {
...
Surface::~Surface() {
    if (_needsfree) ttk_free_surface(_srf);
}
...
void Surface::bitmap(int x, int y, int w, int h, unsigned short* data,
                     ttk_color col) {
...
void Surface::text_lat1(ttk_font fnt, int x, int y, ttk_color col,
                        const char* str) {
    ttk_text_lat1(_srf, fnt, x, y, col, str);
}
void Surface::text_uc16(ttk_font fnt, int x, int y, ttk_color col,
                        const uc16* str) {
...
void Surface::blit(int sx, int sy, int sw, int sh, Surface& dest, int dx,
                   int dy) {
    ttk_blit_image_ex(_srf, sx, sy, sw, sh, dest._srf, dx, dy);
}

Surface Surface::scale(float factor) { return ttk_scale_surface(_srf, factor); }
...
Window::~Window() {
    if (_needsfree) ttk_free_window(_win);
}

void Window::show() { ttk_show_window(_win); }
void Window::hide() { ttk_hide_window(_win); }

void Window::draw() { ttk_draw_window(_win); }

void Window::resize(int mode) {
    switch (mode) {
        case Normal:
            ttk_window_show_header(_win);
            resize(ttk_screen->wx, ttk_screen->wy,
...
void Window::setTitle(const char* str) { ttk_window_title(_win, str); }
const char* Window::title() { return _win->title; }

void Window::addWidget(TWidget* wid) { ttk_add_widget(_win, wid); }
int Window::removeWidget(TWidget* wid) { return ttk_remove_widget(_win, wid); }

int Window::startInput(TWidget* inmethod) {
    return ttk_input_start_for(_win, inmethod);
}
void Window::moveInput(int x, int y) { ttk_input_move_for(_win, x, y); }
void Window::getInputSize(int& w, int& h) { ttk_input_size_for(_win, &w, &h); }

void Window::dirty() { _win->dirty++; }
...
void Widget::setTimer(int ms) { ttk_widget_set_timer(_wid, ms); }
void Widget::setFramerate(int fps) { ttk_widget_set_fps(_wid, fps); }
void Widget::setFramerateInv(int fps_m1) {
    ttk_widget_set_inv_fps(_wid, fps_m1);
}

void Widget::addToHeader() { ttk_add_header_widget(_wid); }
void Widget::removeFromHeader() { ttk_remove_header_widget(_wid); }
...
void Widget::dirty() { _wid->dirty++; }

void Widget::setHoldTime(int holdtime) { _wid->holdtime = holdtime; }

void Widget::setKeyRepeat(int flag) { _wid->keyrepeat = flag; }

void Widget::setRawkeysFlag(int flag) { _wid->rawkeys = flag; }

Window* Widget::window() {
    TWindow* win = _wid->win;
    if (!win->data2) win->data2 = new Window(win);
    return (Window*)win->data2;
}

void inputChar(int ch) { ttk_input_char(ch); }
void endInput() { ttk_input_end(); }

void setTransitionFrames(int frames) { ttk_set_transition_frames(frames); }
void setClicker(void (*fn)()) { ttk_set_clicker(fn); }
void setScrollMultiplier(int num, int denom) {
    ttk_set_scroll_multiplier(num, denom);
}

Timer createTimer(int ms, void (*fn)()) { return ttk_create_timer(ms, fn); }
void destroyTimer(Timer tim) { ttk_destroy_timer(tim); }
...
int buttonPressed(int button) { return ttk_button_pressed(button); }

int iPodVersion() { return ttk_get_podversion(); }

void click() { ttk_click(); }
