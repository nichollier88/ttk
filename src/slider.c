...
// *val must stay allocated until you either set a callback or destroy the
// widget.
...
TWidget* ttk_new_slider_widget(int x, int y, int w, int minval, int maxval,
                               int* val, const char* title) {
    return ttk_new_special_slider_widget(x, y, w, 11, minval, maxval, val,
                                         title);
}

void ttk_slider_set_callback(TWidget* this, void (*callback)(int, int),
                             int cdata) {
    _MAKETHIS;
...
void ttk_slider_draw(TWidget* this, ttk_surface srf) {
    ...
    if (data->label) {
        ttk_text(
            srf, ttk_menufont,
            this->x +
                ((this->w + ttk_text_width(ttk_menufont, data->label)) / 2),
    ...
    ttk_blit_image_ex(data->empty, this->w - (this->w - X), 0, this->w - X, h,
                      srf, this->x + X, y);
}
...
int ttk_slider_scroll(TWidget* this, int dir) {
    ...
    static int sofar = 0;
    sofar += dir;
    if (!(sofar * (data->max - data->min) / 96)) return 0;
    dir = sofar;
    sofar = 0;
...
TWindow* ttk_mh_slider(struct ttk_menu_item* this) {
    ...
    ttk_add_widget(ret, ttk_new_slider_widget(0, 0, data->w, data->min,
                                              data->max, data->val, 0));
