...
int t_GrOpen() {
    if (ttk_init()) return 1;
    return -1;
}

void t_GrClose() { ttk_quit(); }
...
// Imperfection: Amalgamated scroll treated as scroll of 1; scroll gives KEYDOWN
// but not KEYUP.
...
int t_GrGetNextEventTimeout(t_GR_EVENT* ev, int timeout) {
    ...
    if (ttk_getticks() < end) return 0;
    return 1;
}

t_GR_WINDOW_ID t_GrNewWindow(int _u1, int x, int y, int w, int h, int _u2,
                             int _u3, int _u4) {
...
t_GR_WINDOW_ID t_GrNewWindowEx(int _u1, const char* title, int _u3, int x,
                               int y, int w, int h, int _u4) {
    t_GR_WINDOW_ID ret = t_GrNewWindow(0, x, y, w, h, 0, 0, 0);
    ttk_window_title(ret, title);
    return ret;
}

void t_GrDestroyWindow(t_GR_WINDOW_ID w) { ttk_free_window(w); }

void t_GrMapWindow(t_GR_WINDOW_ID w) { ttk_show_window(w); }

void t_GrUnmapWindow(t_GR_WINDOW_ID w) { ttk_hide_window(w); }
...
void t_GrMoveWindow(t_GR_WINDOW_ID win, int x, int y) {
    win->x = x;
    win->y = y;
    if (win->y == 20 || win->y == 21 || win->y == 19) win->y = ttk_screen->wy;
...
t_GR_WINDOW_ID t_GrGetFocus() { return ttk_windows->w; }
...
t_GR_GC_ID t_GrNewGC() { return ttk_new_gc(); }

t_GR_GC_ID t_GrCopyGC(t_GR_GC_ID other) { return ttk_copy_gc(other); }

void t_GrDestroyGC(t_GR_GC_ID gc) { return ttk_free_gc(gc); }
...
void t_GrPoint(t_GR_DRAW_ID dst, t_GR_GC_ID gc, int x, int y) {
    ttk_pixel_gc(dst->srf, gc, x, y);
}
void t_GrLine(t_GR_DRAW_ID dst, t_GR_GC_ID gc, int x1, int y1, int x2, int y2) {
    ttk_line_gc(dst->srf, gc, x1, y1, x2, y2);
}
void t_GrRect(t_GR_DRAW_ID dst, t_GR_GC_ID gc, int x, int y, int w, int h) {
    ttk_rect_gc(dst->srf, gc, x, y, w, h);
}
void t_GrFillRect(t_GR_DRAW_ID dst, t_GR_GC_ID gc, int x, int y, int w, int h) {
    ttk_fillrect_gc(dst->srf, gc, x, y, w, h);
}
void t_GrPoly(t_GR_DRAW_ID dst, t_GR_GC_ID gc, int n, t_GR_POINT* v) {
    ttk_polyline_gc(dst->srf, gc, n, v);
}
void t_GrFillPoly(t_GR_DRAW_ID dst, t_GR_GC_ID gc, int n, t_GR_POINT* v) {
    ttk_fillpoly_gc(dst->srf, gc, n, v);
}
void t_GrEllipse(t_GR_DRAW_ID dst, t_GR_GC_ID gc, int x, int y, int rx,
                 int ry) {
    ttk_ellipse_gc(dst->srf, gc, x, y, rx, ry);
}
void t_GrFillEllipse(t_GR_DRAW_ID dst, t_GR_GC_ID gc, int x, int y, int rx,
                     int ry) {
    ttk_fillellipse_gc(dst->srf, gc, x, y, rx, ry);
}

void t_GrCopyArea(t_GR_DRAW_ID dst, t_GR_GC_ID gc, int dx, int dy, int sw,
                  int sh, t_GR_DRAW_ID src, int sx, int sy, int _unused1) {
    ttk_blit_image_ex(src->srf, sx, sy, sw, sh, dst->srf, dx, dy);
}

void t_GrBitmap(t_GR_DRAW_ID srf, t_GR_GC_ID gc, int x, int y, int width,
                int height, t_GR_BITMAP* imagebits) {
    int minx, maxx;
...
void t_GrDrawImageToFit(t_GR_DRAW_ID dst, t_GR_GC_ID gc, int x, int y, int w,
                        int h, t_GR_IMAGE_ID img) {
    int rw, rh;
    float xf, yf;
...
void t_GrFreeImage(t_GR_IMAGE_ID img) { ttk_free_surface(img); }
...
void t_GrText(t_GR_DRAW_ID dst, t_GR_GC_ID gc, int x, int y, const void* str,
    ...
    if (_u & GR_TFTOP)
        ...
        ttk_text_gc(dst->srf, gc, x, y - ttk_text_height_gc(gc) + 1,
                    (const char*)str);
}

t_GR_FONT_ID t_GrCreateFont(const char* path, int size,
                            struct t_GR_LOGFONT* _u) {
    if (strrchr(path, '/')) path = strrchr(path, '/') + 1;
    return ttk_get_font(path, size);
}

void t_GrDestroyFont(t_GR_FONT_ID font) {}
void t_GrGetGCTextSize(t_GR_GC_ID gc, const void* str, int _u, int flags,
...
void t_GrDestroyTimer(t_GR_TIMER_ID timer) {
    if (timer) ttk_widget_set_timer(timer, 0);
}
